...
function drawShareImage(obj) { //绘制海报方法
	...
	return new Promise((rs, rj) => {
		try {
			...
			if (bgObj && bgObj.path) {
				_app.log('背景有图片路径xxxxxxxxxxxxxxxxxxxxxxxxxxxxx');
				console.log("图片宽度",bgObj)
				let s_h = bgObj.height *0.3
				let s_hh = s_h / bgObj.height
				console.log("图片宽度减少的高度----------------",s_h)
				console.log("图片宽度比例----------------",s_hh)
				// 重新计算商品主图的比例
				// let img_scale = (bgObj.height*0.3)/bgObj.heigh
				Context.drawImage(bgObj.path, 0, 0, bgObj.width, bgObj.height-bgObj.height*0.3);
			} else {
				_app.log('背景没有图片路径');
				if (bgObj.backgroundColor) {
					_app.log('背景有背景颜色:' + bgObj.backgroundColor);
					Context.setFillStyle(bgObj.backgroundColor);
					Context.fillRect(0, 0, bgObj.width, bgObj.height-bgObj.height*0.3);
				} else {
					_app.log('背景没有背景颜色');
				}
			}
...
function drawQrCode(Context, qrCodeObj) { //生成二维码方法， 参考了 诗小柒 的二维码生成器代码
	...
	let options = {
		...
		url:String(qrCodeObj.url || '') || '#ffffff',//二维码图片
		ewcode_size:Number(qrCodeObj.ewcode_size || 0) || 100,//二维码的大小
	}
	let qrCodeAlg = null;
	let d = 0;
	...
	let codeSize = options.ewcode_size
	let ratioImgSize = options.imageSize;
	let ewcode_size = options.ewcode_size//二维码的大小
	//计算每个点的长宽
	let tileW = (ratioSize / count).toPrecision(4);
	let tileH = (ratioSize / count).toPrecision(4);
	//二维码图片
	if (options.url) {
		let x = options.dx + Number(((codeSize - ratioImgSize) / 2).toFixed(2));
		let y = options.dy ;
		drawRoundedRect(Context, x, y, ewcode_size, ewcode_size, 2, 6, true, true)
		Context.drawImage(options.url, x, y, ewcode_size, ewcode_size);
		// 画圆角矩形

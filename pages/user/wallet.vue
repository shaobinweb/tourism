	...
	<view class="wallet">
		<view class="wallet_header">
			<view class="wallet_content">
				<view class="wallet_title">我的钱包(元)</view>
				<view  class="wallet_number">
					<text class="xh">￥</text>
					<text class="number">{{userInfo.data.user_info.user_money || 0}}</text>
				</view>
				<view class="frozen">冻结中: ￥0.00</view>
			</view>
		</view>
		<view class="wallet_mode" v-show="control_id==1">
			<navigator url="/pages/user/balance">
				<view class="mode_option">
					<image src="../../static/meiyong/icon-recharge.png"></image>
					<text>充值</text>
				</view>
			</navigator>
			<navigator url="/pages/user/balanceDh" hover-class="none">
		...
		<view class="wallet_list">
			<view class="wallet_item" v-for="(item,index) in list" :key="index">
				<view class="wallet_item_L">
					<view class="item_L_day">{{item.change_time}}</view>
				</view>
				<view class="wallet_item_M">
					{{item.desc}}
				</view>
				<view class="wallet_item_R">
					<text v-if="item.user_money>0" style="color: #3DB264;">+{{item.user_money}}</text>
					<text v-else style="color: red;">-{{item.user_money}}</text>
				</view>
			</view>
		</view>
	</view>	
...
<script>
	...
	export default{
		...
		data(){
			return{
				id:'all',
				wallet_list:[{id:'all',name:"所有"},{id:'income',name:"收入"},{id:'expenditure',name:"支出"}],
				list : [],
				control_id:1,
				page:1,
				userInfo:null
		...
		methods:{
			...
			async getList(type){
				...
				if( type == 'all'){
					console.log("全部")
					request_api = await request.requestGet('wallet',{page:this.page})
				}
				else{
					console.log("支出或收入")
					request_api = await request.requestGet('wallet',{
						type,
						page:this.page
					})
				}
				...
				let list = data.list
				list.forEach(item=>{
					 var date = new Date(item.change_time*1000);//时间戳为10位需*1000，时间戳为13位的话不需乘1000
					 var Y = date.getFullYear() + '-';
					 var M = (date.getMonth()+1 < 10 ? '0'+(date.getMonth()+1) : date.getMonth()+1) + '-';
					 var D = (date.getDate() < 10 ? '0'+date.getDate() : date.getDate()) + ' ';
					 item.change_time = Y+M+D
				})

	...
	export default {
		...
		async onLoad(options) {
			...
			if(options.type == "product"){
				order_api = await request.requestGet('pieces_buying',{id:options.serial_id})
			}
			console.log("付钱金额",order_api)
		...
		methods: {
			...
			//确认支付
			 confirm: async function() {
				...
				console.log("支付的serial_id",this.serial_id)
				//现在只有两种方式就先这样判断

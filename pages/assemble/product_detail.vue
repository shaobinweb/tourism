	...
	<view :class="{'container':true, 'product':true,'over_body':over_body}">
		...
		<view class="page-bottom">
			...
			<view class="action-btn-group">
				...
				<block v-else>
					...
					<button type="primary" class=" action-btn no-border add-cart-btn" @click="toggleSpec">
						<text class="share_money">￥{{fxzShowMoney}}</text>
					    <label>参与拼团</label>
					</button>
				</block>
		...
		>
			...
			<view class="layer attr-content" @click.stop="stopPrevent">
				...
				<view class="spec_btn">
					<button class="btn" @click="addcar">单独购买</button>
					<button class="btn" @click="buy">参与拼团</button>
				</view>
			</view>
		</view>
		<!-- 分享 -->
		<view class="group_user_box" v-if="group_user">
			<view class="group_user">
				<view class="group_user_close" @tap="group_closefun">
					<image src="../../static/group_btn.png" mode="aspectFit"></image>
				</view>
				<view class="user_header">参与 {{head_user.nickname}} 的拼单</view>
				<view class="user_num_time">仅剩<text style="color: #e4393c;">{{(3 - small_pieces_list.length)||''}}</text>个名额,剩余时间 {{h||''}}时 {{m||''}}分 {{s||''}}秒</view>
				<view class="user_pic_box">
					<block v-if="small_pieces_list.length == 1">
						<view class="user_pic" v-for="(item,index) in small_pieces_list" :key="index">
							<text>团长</text>
							<image :src="item.head_pic.indexOf('127.0.0.1') >=0 ? '../../static/missing-face.png':item.head_pic" mode="aspectFit"></image>
						</view>
						<view class="user_pic">
							<view class="default_user">?</view>
						</view>
						<view class="user_pic">
							<view class="default_user">?</view>
						</view>
					</block>
					<block v-if="small_pieces_list.length == 2">
						<view class="user_pic" v-for="(item,index) in small_pieces_list" :key="index">
					...
					<block v-if="small_pieces_list.length == 3">
						<view class="user_pic" v-for="(item,index) in small_pieces_list">
							<text v-if="index == 0">团长</text>
							<image :src="item.head_pic.indexOf('127.0.0.1') >=0 ? '../../static/missing-face.png':item.head_pic" mode="aspectFit"></image>
						</view>
					</block>
				</view>
				<view class="user_btn" @click="toggleSpec">参与拼团</view>
			</view>
		</view>
	</view>
</template>
	...
	export default{
		...
		data() {
			return {
				...
				p_id:0,//拼团id
				newest_comment:{},
				group_user:true,//控制参团人数弹窗
				h:0,
				m:0,
				s:0,
				set_time:null,//定时器名字
				small_pieces_list:null,//参加拼团人的信息
				head_user:null,//团长的信息
			};
		},
		destroyed() {
			// 页面消失就清除定时器
		...
		async onLoad(options){
			...
			//获取商品详情
			let good_info = await request.requestGet(request.getUrl('group_detail')+'id/'+this.good_id+'/pieces/'+options.pieces+'/group/'+options.group)
			// request.getUrl('spcList')+this.good_id+'/pieces='+options.pieces+'/group='+options.group
			let data = good_info.data.data
			console.log("商品详情网络请求",good_info)
			...
			this.newest_comment = data.newest_comment
			...
			this.lgzShowMoney = data.pieces_goods_price //单独购买的金额
			this.fxzShowMoney = data.goods_price//商品原价
			this.goods_stock = data.goods_stock //库存
			this.small_pieces_list = data.small_pieces_list,//参加拼团人的信息
			this.head_user = data.head_user//团长的信息
			...
			// 拼团规格请求
			let Spec_data = await request.requestGet(request.getUrl('spcList')+this.good_id+'/s=0')
			console.log("拼团规格请求xxxxxxxxxxx",Spec_data) 
			let data2 = Spec_data.data.data
			this.specList = data2.filter_spec//更新规格数据
			this.spec_str = data2.spec_str//修改选中的规格
			this.spec_curr = data2.spec_curr//提交订单需要的商品规格 
		...
		methods:{
			...
			//得到的秒换算成时分秒
			djs: function() { 
				let time =this.small_pieces_list[0].end_time//获取目标时间2020-10-20 10:20:10
			...
			addcar : async function (){
				uni.navigateTo({
					url:`../product/product?id=${this.good_id}`
				})
			...
			buy(){
				let _this = this
				_this.verifyFun()
				let user_id = uni.getStorageSync('ebizUuid')//用户id和拼团列表里面的列表比较，判断是否在该团
				let user_if = false
				_this.small_pieces_list.forEach((item,index)=>{
					if( user_id == item.user_id){
						console.log("你已经在该团了",user_id)
						user_if = true
					}
				})
				if(user_if){
					_this.$api.msg('你已加入该拼团')
					return
				}
				setTimeout(function(){
					...
					if(_this.bind_phone){
						uni.navigateTo({
							url: `./createOrder?goods_id=${_this.good_id}&goods_spec=${_this.spec_curr}&goods_num=${_this.good_number}&type=product&p_id=${_this.p_id}`
					})
					_this.toggleSpec()
					}
				},300)
			...
			//判断是否被收藏
			 ...
			 add_cancelFun : async function ( type ){
				 ...
				 _this.verifyFun()
					 ...
					 //登陆才能收藏的相关操作
				 if( type == 'add' ){
					let cancel_add = await request.requestPost('add_collect',{
				...
				if ( type == 'cancel' ){
				    console.log("取消收藏")
					let cancel_add = await request.requestPost('cancel_collect',{
					 		goods_id : _this.good_id
					})
			...
			async selectSpec(sid){
			   console.log("规格的id=============xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx",sid)
			   let _this = this
			   let Spec_data = await request.requestGet(request.getUrl('spcList')+this.good_id+'/s/'+sid)
			   console.log("切换规格",Spec_data)
...
<style lang='scss'>
	/* 拼团人数弹窗start */
	.group_user_box{ width:100%;height: 100%;position: fixed;top:0px;bottom:0rpx;left:0rpx;right:0px;z-index:1000;display: flex;justify-content: center;align-items: center;background: rgba(0,0,0,0.4);
		.group_user{ width:500rpx;height:400rpx;background: #fff;border-radius: 16rpx;padding:40rpx 30rpx;display: flex;justify-content: space-between;flex-direction: column;align-items: center;position: relative;
			.user_header{ font-size:32rpx;color:#333}
			.user_num_time{ font-size: 24rpx;color: #666;}
			.user_pic_box{ width:100%;display: flex;justify-content: center;align-items: center;
				.user_pic{ width:94rpx;height: 94rpx;position: relative;margin-right:10rpx;
					text{ position: absolute;left:-36rpx;top:0px;width:72rpx;height: 32rpx;background: #e4393c;display: flex;justify-content: center;align-items: center;color: #fff;font-size: 20rpx;z-index: 200;border-radius: 200rpx;}
					image,.default_user{ width:94rpx;height: 94rpx;border-radius: 100%;margin-right:10rpx}
					.default_user{ display: flex;align-items: center;justify-content: center;font-size: 48rpx;background: #f5f5f5;color: #999;}
				}
			}
			.user_btn{ width:434rpx;height: 70rpx;display: flex;justify-content: center;align-items: center;background: #e4393c;color: #fff;font-size: 28rpx;border-radius: 198rpx;}
			.group_user_close{ width:48rpx;height: 48rpx;position: absolute;right:-24rpx;top:-24rpx;background: #fff;border-radius: 100%;overflow: hidden;

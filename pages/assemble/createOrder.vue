	...
	<view>
				...
				<view class="g-item" v-for="item in items" :key="item.goods_id">
					...
					<view class="right">
						...
						<view class="price-box">
							<text class="price">￥{{item.shop_price}} + {{item.cecn_num}}</text>
		...
		<view class="message_box">
			...
			<view class="message_box_m message_list">
				...
				<view class="message_m_r" @click.stop="invoiceFun">
					...
					<block v-if="invoice_type == 0">个人发票</block>
					<block v-if="invoice_type == 1">企业发票</block>
					<text class="yticon icon-you" style="font-size: 28rpx;"></text>
				</view>
			</view>
			<!-- @click="toggleMask('show')" -->
			<view class="message_box_b message_list" >
				<view class="message_b_l">优惠券</view>
				<block  v-if="concessions_price == 0">
					<view class="message_b_r">
						<!-- {{couponList_length}} 张优惠券可用 -->
						<label  style="color: #e4393c;">没有可选优惠券</label> 
					</view>
				</block>
				<block v-else>
					<view class="message_b_r">
		...
		<view class="statistics">
			...
			<view class="sta_list">
				...
				<view class="sta_list_r">¥ {{c_total_price || 0}}</view>
			</view>
			<view class="sta_list">
		...
		<view class="footer">
			<view class="price-content">
				...
				<text class="price">￥{{total_c_price || 0}}+{{order_payment_data.cecn_num || 0}}购物金</text>
			</view>
		...
		<view class="coupon" v-if="maskState == 1">
			<view class="coupon_box">
				<view class="coupon_header">
					...
					<view class="c_header_right" @click="toggleMask"><image src="../../static/c_close.png" mode="aspectFit"></image></view>
				</view>
				<view class="coupon_type">
					<view class="type_available c_active">可用优惠卷（{{couponList_length}}）</view>
					<view class="type_no_ava">不可用优惠卷（0）</view>
				</view>
	...
	export default {
		...
		data() {
			return {
				...
				goods_data : [],
				select_coupon : [],
				c_total_price:0,//优惠合计  与concessions_price不同
		...
		methods: {
			...
			select_couponFun(type,event){
				...
				if(type == 0){
					...
					Object.keys(_this.couponList[0]).forEach(function(key){
						//判断是否可以多选  coupon_level为2只能单选，coupon_level为1可以多选
						if(_this.couponList[0][key].coupon_level == 2 && coupon_level == 2){
							//把单选的优惠卷选中取消
			...
			get_concessions(){
				...
				Object.keys(_this.couponList[0]).forEach(function(key){
					 ...
					 if(_this.couponList[0][key].is_select == 1  ){
						 ...
						 if(_this.couponList[0][key].discount_type == 1){
							 console.log(typeof _this.total_c_price)
							 console.log("打折的金额",_this.total_c_price)
						 	let n = Number(_this.couponList[0][key].quota)
							let discount = _this.total_c_price * [1-(n *0.1)]//用总金额去打折
				...
				let _price = parseFloat(_this.concessions_price)//优惠价格
				_this.total_c_price = (parseFloat(_this.order_payment_data.total_price) - _price).toFixed(2)
				_this.c_total_price =(parseFloat(order_payment_data.discount_price) + parseFloat(concessions_price)).toFixed(2) //优惠合计
			...
			async getFun(){
				...
				if(this.type == "product"){
					//商品详情页过来的
					// 创建订单
					let buy_api = await request.requestGet('buying',{
					...
					this.total_c_price = parseFloat(this.order_payment_data.total_price)
					Object.keys(_this.couponList[0]).forEach(function(key){
			...
			async submit(){
				...
				if( this.type =="product"){
					...
					console.log("拼团idxxxxxxxxxxxxxxxxxxx",this.p_id)
					let post_data
					if( Vue.prototype.$invoice == -1){
						post_data = {
						  "pieces":this.p_id,
					...
					let submit_post = await request.requestPost('pieces',post_data)
					let data = submit_post.data.data
					console.log("拼团下单提交的参数",post_data)
					console.log("拼团",submit_post)
					uni.redirectTo({
						url:`/pages/assemble/pay?serial_id=${data.pieces_id}&type=product`
					})
				}
...
<style lang="scss">
	...
	.coupon{ width:100%;height:100vh;position: fixed;z-index:200;left:0px;right:0px;top:0px;bottom:0px;background: rgba(0,0,0,0.4);z-index:2000;
		.coupon_box{ width:100%;height:80vh;background: #fff;border-radius: 10px 10px 0px 0px;position: fixed;left:0px;right:0px;bottom:0px;z-index:2000;padding-bottom:40px;overflow-x: auto;
			...
			.list_box_c{ width:100%;height: 700rpx;overflow: auto;}
	...
	// 商品统计
	.statistics{ width:100%;height: auto;background: #fff;margin-top:16rpx;padding:18rpx;font-size: 24rpx;display: flex;flex-direction: column;justify-content: space-between;border-radius: 10rpx;color: #666;margin-bottom:140rpx}
